...
void uart_interrupt_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)uart_interrupt__INTC_NUMBER] = address;
}
...
cyisraddress uart_interrupt_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)uart_interrupt__INTC_NUMBER];
}
...
{
    *uart_interrupt_INTC_PRIOR = priority << 5;
}
...
uint8 uart_interrupt_GetPriority(void)
{
    ...
    priority = *uart_interrupt_INTC_PRIOR >> 5;
